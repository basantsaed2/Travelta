...
const BookingEngine = ({ update, setUpdate }) => {
  ...
  useEffect(() => {
    ...
    const countries = countryData?.countries?.map((c) => ({ type: "Country", name: c.name, id: c.id })) || [];
    const cities = cityData?.cities?.map((ci) => ({ type: "City", name: ci.name, id: ci.id })) || [];
    // Tours are not part of the search input dropdown for hotels/cities/countries
    // const tours = tourData?.tourtype?.map((t) => ({ type: "Tour", name: t.name, t: t.id })) || [];
  ...
  useEffect(() => {
    if (selectedCountry && cityData?.cities) {
      const citiesForCountry = cityData.cities.filter(
        (city) => city.country_id === selectedCountry.id
      );
      setFilteredCities(cityData?.cities);
      setSelectedCity(null); // Reset selected city when country changes
    } else {
      setFilteredCities(cityData?.cities || []); // Show all cities if no country selected initially
    }
  }, [selectedCountry, cityData]);


  // Update hotels only when Room tab is active and response is received
  useEffect(() => {
    if (!loadingSearchRoom && responseSearchRoom && tabValue === 0) {
      setHotels(responseSearchRoom?.data?.hotels);
    } else if (tabValue !== 0) {
      setHotels([]); // Clear hotels if tab changes away from Room
    }
  }, [responseSearchRoom, tabValue]); // Added tabValue to dependencies

  // Update tours only when Tour tab is active and response is received
  useEffect(() => {
    if (!loadingSearchTour && responseSearchTour && tabValue === 1) {
      setTours(responseSearchTour?.data?.tours);
    } else if (tabValue !== 1) {
      setTours([]); // Clear tours if tab changes away from Tour
    }
  }, [responseSearchTour, tabValue]); // Added tabValue to dependencies


  const handleTabChange = (_, newValue) => {
    setTabValue(newValue);
    // Optionally clear results when switching tabs
    setHotels([]);
    setTours([]);
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (tabValue === 0) {
      ...
      postRoom(formData);
      setTours([]); // Clear tour data when searching for rooms
    } else if (tabValue === 1) {
      // Ensure selectedCountry, selectedCity, selectedTour are not null before appending
      if (selectedCountry) formData.append("destination_country", selectedCountry.id);
      if (selectedTour) formData.append("tour_type_id", selectedTour.id);
      if (selectedCity) formData.append("destination_city", selectedCity.id);
      ...
      postTour(formData);
      setHotels([]); // Clear hotel data when searching for tours
    }
  };
  ...
  return (
    <>
        ...
        {tabValue === 1 && (
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-6 gap-4">
            ...
            <Autocomplete
              ...
              getOptionLabel={(option) => option.name}
              value={selectedTour}
              onChange={(_, newValue) => setSelectedTour(newValue)}
              renderInput={(params) => <TextField {...params} label="Select Tour" fullWidth />}
            />
            <TextField
              fullWidth
      ...
      {tabValue === 1 && responseSearchTour &&
        tours.length === 0 ? (
        <div className="flex justify-center items-center p-6">
          <p className="text-lg text-gray-600">Unfortunately, no tours match your search criteria. Please try again with different filters.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 md:p-6 p-2 pt-0">
          {tours.map((tour) => (
            <div
              key={tour.id} // Changed to tour.id as per the provided JSON structure
              className="bg-mainBgColor rounded-xl shadow-xl overflow-hidden transition-transform transform hover:scale-90"
            >
              <div className="w-full p-4">
                {/* Tour Header */}
                <div className="w-full flex gap-4">
                  ...
                  <div className="w-full flex flex-col">
                    ...
                    {/* Display description with a clamp for multi-line text */}
                ...
                <div className="space-y-3 mt-3">
                  ...
                  {/* Nights & Price */}
                  <div className="flex justify-between items-center text-gray-700">
                    {tour.nights !== undefined && ( // Check if nights exists
                      <div className="flex items-center gap-2">
                        <MdNightsStay className="text-lg text-gray-700" />
                        <span className="text-sm"><strong>Nights:</strong> {tour.nights}</span>
                      </div>
                    )}
                    {/* Using tour_pricings to get price, assuming it's the first pricing item. Adjust if logic is different. */}
                    {tour.tour_pricings && tour.tour_pricings.length > 0 && tour.tour_pricings[0].tour_pricing_items && tour.tour_pricings[0].tour_pricing_items.length > 0 && (
                      <div className="flex items-center gap-2 font-bold">
                        <FaRegMoneyBillAlt className="text-lg" />
                        <span>{tour.tour_pricings[0].tour_pricing_items[0].price} {tour.tour_pricings[0].tour_pricing_items[0].currency?.name || 'USD'}</span>
                      </div>
                    )}
                  </div>
