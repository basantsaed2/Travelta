...
const EditManualBookingPage = () => {
  ...
  const [isManualServiceChange, setIsManualServiceChange] = useState(false);
  ...
  //To track the tour details
    ...
    useEffect(() => {
      ...
      refetchSuppliers();
      refetchBookingData();
    }, [refetchBookingList, refetchSuppliers,refetchBookingData, update]);


    useEffect(() => {
        if (dataBooking && dataBooking.manuel_booking) {
          console.log("Booking Data:", dataBooking.manuel_booking);
          const bookingData = dataBooking.manuel_booking;

          setIsManualServiceChange(false); // <-- Important line here
          setSelectedService(bookingData.service);

          if(bookingData.to_supplier_id && bookingData.to_supplier) {
          ...
          if(bookingData.agent_sales_id && bookingData.agent_sales) {
            setSelectAgent(bookingData.agent_sales);  
          }
          if(bookingData.country_id && bookingData.country) {
            setSelectedCountry(bookingData.country_id);  
          }
          if(bookingData.currency_id && bookingData.currency) {
            setSelectedCurrency(bookingData.currency);  
          }
          setCost(bookingData.cost || 0);
          setPrice(bookingData.price || 0);
          setTotalPrice(bookingData.total_price || 0);
          setMarkupValue(bookingData.mark_up || 0);
          if (bookingData.mark_up_type === "precentage") {
            setIsMarkupPercentage(1); // Set to percentage
          }else if (bookingData.mark_up_type === "value") {
            setIsMarkupPercentage(0); // Set to value
          }
          setSelectedTaxType(bookingData.tax_type || ""); // Set the selected tax type
          if(bookingData.taxes && bookingData.taxes.length > 0) {
            const taxIds = bookingData.taxes.map((tax) => tax.id); // Extract IDs from the tax objects
            setSelectedTaxId(taxIds); // Set the selected tax IDs
          } else {
          ...
          if (bookingData.hotel && bookingData.service?.service_name === "Hotel") {
            ...
            setVisaCountry(bookingData.visa.country || "");
            setVisaTravelDate(bookingData.visa.travel_date || "");
            setVisaAppointmentDate(bookingData.visa.appointment_date || "");
            ...
            setSelectedFlightType(bookingData.flight.type || "");
            setSelectedFlightDirection(bookingData.flight.direction || "");
            setFlightDeparture(bookingData.flight.departure || "");
            setFlightArrival(bookingData.flight.arrival || "");
            setFlightAdultPrice(bookingData.flight.adult_price || "");
            ...
            setSelectedTourType(bookingData.tour.type || "");
            setTourAdultPrice(bookingData.tour.adult_price || "");
            setTourChildPrice(bookingData.tour.child_price || "");
            setTourAdultsNumber(bookingData.tour.adults || '');
            setTourChildrenNumber(bookingData.tour.childreen || '');
          }
        
        }
      }, [dataBooking]); // Only run this effect when `data` changes
    ...
    useEffect(() => {
      if (selectedService && selectedService.id) {
        if (isManualServiceChange) {
          setSelectedFromSupplier(null); // Clear only on manual change
        }
    ...
    useEffect(() => {
      if (responseCustomerServicesData && !loadingPostCustomerServices) {
          ...
          <div className="grid sm:grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <TextField
              ...
              onChange={(e) => {
                ...
                setIsManualServiceChange(true); // <-- ADD THIS LINE
              }}              
...
export default EditManualBookingPage;
