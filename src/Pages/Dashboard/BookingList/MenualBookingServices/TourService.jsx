...
const TourService = ({ data }) => {
    ...
    useEffect(() => {
        ...
        // Date range filter - using hotel check-in date as reference
        if (startDate && endDate) {
            filtered = filtered.filter(tour => {
                if (tour.tour_hotels && tour.tour_hotels.length > 0) {
                    const checkInDate = new Date(tour.tour_hotels[0].check_in);
    ...
    const currentRows = filteredTours.slice(indexOfFirstRow, indexOfLastRow);
    ...
    const exportToExcel = () => {
        const worksheet = XLSX.utils.json_to_sheet(
            tours.map((tour, index) => ({
                ...
                Adults: tour.adults_no || "-",
                Children: tour.children_no || "-",
                Status: tour.status || "-",
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "Tour_Bookings");
        XLSX.writeFile(workbook, "Tour_Bookings.xlsx");
    };
                                ...
                                {currentRows.length === 0 ? (
                                    ...
                                    currentRows.map((tour, index) => (
                                        ...
                                        >
                                            ...
                                            <td className="text-center py-2 text-gray-600">{tour?.tour_name || "-"}</td>
                                            <td className="text-center py-2 text-gray-600">{tour?.country || "-"}</td>
                                            <td className="text-center py-2 text-gray-600">{tour.tour_hotels.map((hotel, index) => (
                                                <span key={index}>
                                                    {hotel.check_in}
                                                </span>
                                            ))}</td>
                                            <td className="text-center py-2 text-gray-600">{tour.tour_hotels.map((hotel, index) => (
                                             ...
                                             <td className="text-center py-2 text-gray-600">{tour.tour_buses.map((bus, index) => (
                                                <span key={index}>
                                                    {bus.transportation}
                                                </span>
                                            ))}
                                            ...
                                            <td className="text-center py-2">
                                                <span className={`px-2 py-1 rounded text-xs ${tour.status === 'confirmed' ? 'bg-green-100 text-green-800' :
                                            ...
                                            <td className="text-center py-2">
                                                <span className={`px-2 py-1 rounded text-xs ${tour.payment_status === 'paid' ? 'bg-green-100 text-green-800' :
                                            ...
                                            <div className="flex items-center justify-center gap-1">
                                             ...
                                             <td className="text-center flex items-center justify-center mt-3 py-2 text-gray-600">
                                                <Link to={`/dashboard_agent/booking/manual_booking/edit_booking/${tour.id}`}  ><FaEdit color='#4CAF50' size="24"/></Link>
                                                </td> 
                                        </tr>
                                    ))
                                )}
