...
const DetailsUpcoming = () => {
  ...
  useEffect(() => {
    if (currentData) {
      setCurrentList(currentData.upcoming || []);
    }
    console.log(currentData?.upcoming[type]);
  }, [currentData, type]);
...
const handleUpdateRequest = async () => {
  ...
  try {
    const response = await fetch(`https://travelta.online/agent/booking/special_request/${upcoming_id}`, {
      method: "PUT",
      headers: {
...
{/* Status & Special Request Card */}
<div className="p-5 border border-gray-200 shadow-lg rounded-lg bg-gray-200">
  <h3 className="text-xl font-semibold text-gray-800 border-b pb-2">🔖 Status & Special Request</h3>
  <div className="mt-3">
    <span
