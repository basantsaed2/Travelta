...
const EditRequestPage = () => {
  ...
  const {postData: postDataFlight,loadingPost: loadingPostFlight,response: responseFlight,} = usePost({ url: `https://travelta.online/agent/request/update_flight/${requestId}` });
  const {postData: postDataTour,loadingPost: loadingPostTour,response: responseTour,} = usePost({ url: `https://travelta.online/agent/request/update_tour/${requestId}` });
  ...
  useEffect(() => {
    refetchList();
    refetchRequestData();
  }, [refetchList,refetchRequestData]);
  ...
  useEffect(() => {
    if (dataRequest && dataRequest.request) {
      const request=dataRequest.request;
      setSelectedService(request.service);
      setSelectCustomer(request.customer);
      setSelectedAgent(request.admin_agent);
