...
const HistoryVisa = ({ data, loading,refetch }) => {
      ...
      <table className="w-full sm:min-w-0">
        <thead className="w-full">
          <tr className="w-full border-b-2">
            {[
              "Agent", "Appointment", "Country", "Currency", "Revenue", 
              "Adults", "Children", "Priority", "Service", "Stages", 
              "To Name", "To Phone", "Visa Notes","Action"
            ].map((heading) => (
              <th
                key={heading}
          ...
          {filteredData.length > 0 ? (
            filteredData.slice(0, rowsToDisplay).map((row, index) => (
              ...
              >
                ...
                <td className="min-w-[80px] sm:min-w-[50px] sm:w-1/12 lg:w-1/12 py-2 text-center text-thirdColor text-sm sm:text-base lg:text-lg xl:text-xl overflow-hidden">{row.appointment || "N/A"}</td>
                <td className="min-w-[80px] sm:min-w-[50px] sm:w-1/12 lg:w-1/12 py-2 text-center text-thirdColor text-sm sm:text-base lg:text-lg xl:text-xl overflow-hidden">{row.country_name || "N/A"}</td>
