      ...
      <div className="w-full custom-scrollbar overflow-x-auto rounded-lg shadow-md bg-white p-2">
        <table className="w-full sm:min-w-0">
          ...
          <tr className="border-b-2">
            {["SL", "Client Name", "Client Phone", "Agent", "Revenue", "Arrival", "Departure", "Bus Name", "Bus No.", "Driver Phone","From → To", "Adults", "Children", "Priority", "Service", "Stages", "Notes", "Action"].map(h => (
              <th
                key={h}
          ...
          <tbody>
            {filtered.length > 0 ? filtered.slice(0, rowsToDisplay).map((row,index) => (
              <tr key={row.id} className="even:bg-gray-100 hover:bg-gray-200 text-base">
                ...
                <td className="text-center py-2">{row.arrival || "N/A"}</td>
                <td className="text-center py-2">{row.depature || "N/A"}</td>
                <td className="text-center py-2">{row.bus_name}</td>
                <td className="text-center py-2">{row.bus_no}</td>
                <td className="text-center py-2">{row.driver_phone}</td>
