...
import { CircularProgress, MenuItem, TextField, DialogTitle, DialogContent, DialogActions } from '@mui/material';
...
const BusTable = ({ data = [], loading, refetch, priorityOptions = [], stageOptions = [], isHistory = false }) => {
  ...
  useEffect(() => {
    ...
    // Apply date range filter (compares dates only, ignoring time)
    if (startDate || endDate) {
      result = result.filter(r => {
        // Extract date part from departure (reset time to 00:00:00 for accurate comparison)
        const departureDate = new Date(r.depature);
        departureDate.setHours(0, 0, 0, 0); // Normalize to date-only

        // Normalize start/end dates (if provided)
        ...
        // Compare dates
        const afterStart = !start || departureDate >= start;
        const beforeEnd = !end || departureDate <= end;
      ...
      <table className="w-full min-w-[600px]">
        <thead className="bg-gray-200 text-gray-700">
          <tr className="border-t-2 border-b-2">
            ...
            {["Client Name", "Client Phone", "Agent", "Revenue", "Departure", "Arrival", "Bus Name", "Bus No.", "From → To", "Driver Phone", "Priority", "Stage", "Notes", "Actions"].map((name, index) => (
              <th
              key={index}
        ...
        <tbody>
          {paginatedData.length === 0 ? (
            <tr>
              <td colSpan={14} className="text-center text-xl text-gray-500 py-4">
                No Buses Found
              </td>
            </tr>
          ) : (
            paginatedData.map((row, index) => (
              ...
              >
                ...
                <td className="text-center py-2 text-gray-600">{row.depature || "-"}</td>
                <td className="text-center py-2 text-gray-600">{row.arrival || "-"}</td>
                <td className="text-center py-2 text-gray-600">{row.bus_name || "-"}</td>
                <td className="text-center py-2 text-gray-600">{row.bus_no || "-"}</td>
                <td className="text-center text-sm py-2">
                  {row.from} <span className="text-mainColor font-semibold">→</span> {row.to}
                </td>
                <td className="text-center py-2 text-gray-600">{row.driver_phone || "-"}</td>
    ...
    >
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <DialogPanel className="w-full max-w-xl rounded-2xl bg-white shadow-xl flex flex-col max-h-[90vh]">
          {/* Modal Header */}
          <div className="p-6 pb-0 sticky top-0 bg-white z-10 border-b border-gray-200">
            <div className="flex items-center justify-between mb-4">
              <DialogTitle className="text-2xl font-bold text-gray-900">
...
const BusesRequest = () => {
  ...
  const loading = loadingBus || loadingList;
  const currentData = Bus?.current?.buses || [];
  const historyData = Bus?.history?.buses || [];
