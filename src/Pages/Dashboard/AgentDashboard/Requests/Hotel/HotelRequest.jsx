...
const HotelTable = ({ data = [], loading, refetch, priorityOptions = [], stageOptions = [], isHistory = false }) => {
        ...
        <table className="w-full min-w-[600px]">
          <thead className="bg-gray-200 text-gray-700">
            <tr className="border-t-2 border-b-2">
              ...
              {["Client Name", "Client Phone", "Agent", "Revenue", "Check In", "Check Out", "Hotel Name", "Priority", "Stage", "Notes", "Actions"].map((name, index) => (
                <th
                  key={index}
            ...
            {paginatedData.length === 0 ? (
              ...
              paginatedData.map((row, index) => (
                ...
                >
                  ...
                  <td className="text-center py-2 text-gray-600">{row.check_in || "-"}</td>
                  <td className="text-center py-2 text-gray-600">{row.check_out || "-"}</td>
                  <td className="text-center py-2 text-gray-600">{row.hotel_name || "-"}</td>
...
const HotelRequest = () => {
  ...
  const currentData = (Hotel?.current?.hotels && Array.isArray(Hotel.current.hotels)) ? Hotel.current.hotels : [];
  const historyData = (Hotel?.history?.hotels && Array.isArray(Hotel.history.hotels)) ? Hotel.history.hotels : [];
