...
const ExpenseListPage = ({ update, setUpdate }) => {
    const { refetch: refetchExpenses, loading: loadingExpenses, data: expenseData } = useGet({
    ...
    const filteredExpenses = expenses.filter((expense) => {
        ...
        const financialMatch = selectedFinancial ? 
            expense.financial?.name === selectedFinancial : true;

        return matchesSearch && financialMatch;
    });

    // Get unique financial accounts
    const uniqueFinancials = [...new Set(expenses.map(expense => expense.financial?.name).filter(Boolean))];
    ...
    const exportToExcel = () => {
        const worksheet = XLSX.utils.json_to_sheet(
            filteredExpenses.map((expense, index) => ({
                ...
                Financial_Account: expense.financial?.name || "-",
                                        ...
                                        >
                                            ...
                                            <td className="text-center py-2 text-gray-600 relative">
                                                <span className="block max-w-[150px] truncate mx-auto cursor-pointer group">
                                                    {expense?.title || "-"}
                                                    {expense?.title && expense.title.length > 15 && (
                                                                        ...
                                                                        <div className="flex flex-col items-center justify-center bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                                                                            ...
                                                                            <div className="flex items-center">
                                                                                <div className="mt-2 text-center">
                                                                                    You will delete expense: {expense?.title || "-"}
                                                                                </div>
                                                                            </div>
                                                                        </div>
