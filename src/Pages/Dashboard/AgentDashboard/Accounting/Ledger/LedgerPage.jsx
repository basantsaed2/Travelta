...
const LedgerPage = ({ update, setUpdate }) => {
  ...
  const [filteredLedgers, setFilteredLedgers] = useState([]); // Store filtered results
  ...
  // Fetch ledger data on mount and update
  useEffect(() => {
    refetchLedger();
  ...
  useEffect(() => {
    if (ledgerData) {
      ...
      setFilteredLedgers(allLedgers); // Default to full dataset
    }
  }, [ledgerData]);
  ...
  useEffect(() => {
    ...
    if (startDate && endDate) {
      filtered = filtered.filter(ledger => {
        ...
        return ledgerDate >= new Date(startDate) && ledgerDate <= new Date(endDate);
      });
    }

    setFilteredLedgers(filtered);
    setCurrentPage(1); // Reset to first page on filter change
  }, [searchText, selectedType, startDate, endDate, ledgers]);
  ...
  const currentRows = filteredLedgers.slice(indexOfFirstRow, indexOfLastRow);
  ...
  // Render ledger details inside modal
