...
{openUpdate && selectedExpense && (
  <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center z-50">
    <div className="bg-white p-6 rounded-lg shadow-lg w-96">
      ...
      {/* Expense Table */}
      <table className="w-full border mt-5 shadow-lg rounded-lg overflow-x-auto">
  <thead>
      ...
      <tr key={expense.id} className={`text-center ${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'} hover:bg-gray-100 transition duration-200`}>
        ...
        <td className="p-4 border-b text-sm">
          <div className="flex justify-center space-x-2">
            <IconButton onClick={() => handleOpenUpdate(expense)} className="text-blue-600 hover:text-blue-800 transition duration-150">
              <FaEdit />
            </IconButton>
            <IconButton onClick={() => handleDelete(expense.id)} className="text-red-600 hover:text-red-800 transition duration-150">
              <FaTrashCan />
            </IconButton>
          </div>
        </td>
