...
const EditRoomPricingPage =({ update, setUpdate })=>{
    const { pricingId } = useParams();
    const { roomId } = useParams();
    
    const { refetch: refetchPricingData, loading: loadingPricingData, data: pricing } = useGet({ url:`https://travelta.online/agent/room/pricing/item/${pricingId}`});
    ...
    const { postData, loadingPost, response } = usePost({url:`https://travelta.online/agent/room/pricing/update/${pricingId}`,});
    ...
    useEffect(() => {
        if (pricing && pricing.pricing) {
            const pricingRoom = pricing.pricing;
            setName(pricingRoom.name)
            setFromDate(pricingRoom.from || '')
            setToDate(pricingRoom.to || '')
            setPrice(pricingRoom.price || '')
            setSelectedCurrency(pricingRoom.currency_id || '')
            setSelectedPricingData(pricingRoom.pricing_data_id || '')

            if(pricingRoom.nationality.length > 0 && pricingRoom.groups.length > 0){
                setNationalityType('nationality&group')
                const selectedNationalityIds = pricingRoom.nationality.map(nationality => nationality.id)
                const selectedGroupIds = pricingRoom.groups.map(group => group.id)
                setSelectedNationality(selectedNationalityIds)
                setSelectedGroup(selectedGroupIds)
            }else if(pricingRoom.nationality.length>0){
                setNationalityType('nationality')
                const selectedNationalityIds = pricingRoom.nationality.map(nationality => nationality.id)
                setSelectedNationality(selectedNationalityIds)
            }else if(pricingRoom.groups.length>0){
                setNationalityType('group')
                const selectedGroupIds = pricingRoom.groups.map(group => group.id)
                setSelectedGroup(selectedGroupIds)           
            }
        }
    ...
    return(
        <>
        {loadingPost || loadingPricing || loadingPricingData ? (
            <>
