...
const SignUpSupplier = () => {
  ...
  useEffect(() => {
    if(countryData && cityData && sourceData && serviceData){
      ...
      const formattedServiceOptions = serviceData.map((service) => ({
        value: service.name, // Use the country name or ID as the value
        label: service.name, // Display the country name
      }));
  ...
  const handleServiceChange = (selected) => {
    setSelectedServiceOption(selected);
    console.log('Selected Service:', selected); // Do something with the selected option
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        // Add service IDs (as comma-separated or array)
 ...
 if (!data) {
  ...
  return (
    <div className="sign-up-page flex flex-col items-center justify-center lg:justify-around w-full xl:h-screen lg:h-full text-secoundColor">
        <h2 className="p-4 pb-0 text-3xl font-semibold text-mainColor text-center mb-5">SignUp Supplier</h2>        
